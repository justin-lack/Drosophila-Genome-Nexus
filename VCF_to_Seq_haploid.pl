...
my $quality = 75; #set this to the quality threshold of your choosing
...
while (<Y>){
  ...
  if ($PositionX == 1) {
    ...
    while (<X>){
      ...
      @line2 = split;
      next if ($line2[0] =~ m/#/);
      last if ($line2[0] > $RefChrom);
      if ($line2[0] == $RefChrom){
	push @IndelChromX, $line2[0];
	push @IndelPositionX, $line2[1];
	push @IndelRefX, length$line2[3];
	push @IndelAltX, length$line2[4];
      }
    }
    $pos++;
    close X;
    print scalar @IndelChromX . "\n";
  }
  ...
  if ($a < @IndelChromX) {
	...
	$a++;
      ...
      if ($PositionX <= ($IndelPositionX[$a] + $buffer + ($IndelRefX[$a]))) {
      }
      else {
	$Chrom2 = $ChromX;
	$Position2 = $PositionX;
...
#Now, lets write sequences
  if ($m == 0) {
    if (($Qual2 ne '.') && ($Qual2 >= 75) && ($Alt2 eq '.')){
      print C "$Ref2";
    }
    elsif (($Qual2 ne '.') && ($Qual2 >= 75) && ($Alt2 ne '.') && (($AlleleAlt_Depth2/($AlleleAlt_Depth2 + $AlleleRef_Depth2)) > 0.75)){
      print C "$Alt2";
    }
    else {
      print C "N";
