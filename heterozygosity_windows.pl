#This script generates the heterozygosity windows used as input for the script heterozygosity_to_mask.pl.
...
#INPUT
  ...
  while (<V>){
	...
	if ($Genotype ne 'X') {
	  $sites++;
	  if (($Genotype eq '0/1') || ($Genotype eq '1/2')) {
	    $hets++;
	  }
	}
	$prevpos = $Position;
...
#	print "window ends at $prevpos\n";
  ...
  close V;
  $cmd = 'gzip ' . $VCFin[$z] . 'diploid_shifted.vcf';
  system($cmd);
  $cmd = 'purge';
  system($cmd);
